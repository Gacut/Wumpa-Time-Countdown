...
class WumpaTime(App):
	def build(self):
		...
		self.lista = Label(text='time left: ' + str(mainTime)[2:10].replace('.',':'),
						   font_size='100dp', font_name='ComicsCarToon.ttf', 
						   outline_color=[0,0,0], outline_width=2)

		self.t = TextInput(font_size=200, size_hint_y=None, height=200, halign='right', 
						   input_filter='int', multiline=False, background_color=[1,1,1,0],
						   foreground_color=[1,1,1,1], font_name='ComicsCarToon.ttf', 
						   hint_text='--:--:--', hint_text_color=[56, 64, 201,1])

		self.badd = Button(size_hint=(None, None),size=(200, 200), on_release=self.update_label, 
						   background_normal='Add1.png', background_down='Add2.png')

		breset = Button(size_hint=(None, None), size=(200, 200), on_release=self.reset_button, 
			            background_normal='Reset1.png', background_down='Reset2.png')

		bremove = Button(pos=(0, 200) ,size_hint=(None, None), size=(200, 200), on_release=self.rem_last,
						 background_normal='Remove1.png', background_down='Remove2.png')
	...
	def update_label(self, button_instance):
		...
		try:
			...
			czas = datetime.timedelta(minutes=minutes, seconds=seconds, milliseconds=milliseconds)
